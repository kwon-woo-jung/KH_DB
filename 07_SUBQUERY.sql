...
-- 3) 위의 2단계를 하나의 쿼리로!
-- 전 직원의 평균 급여보다 많은 급여를 받고 이쓴ㄴ 직원의
-- 사번, 이름, 직급코드, 급여 조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
WHERE SALART > (SELECT AVG(SALARY) FROM EMPLOYEE)
ORDER BY JOB_CODE;
...
-- 메인쿼리 + 서브쿼리
SELECT EMP_ID, JOB_NAME, JOB_NAME, DEPT_CODE, SALARY, HIRE_DATE
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
-- 3) 대리 직급의 직원들 중에서과장 직급의 최소 급여보다
-- 많이 받는 직원의 사번, 이름, 직급명, 급여 조회


-- 방법 1) MIN 을 이용해서 단일행 서브쿼리 만듦.
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리'
...
-- 2) DEPARTMENT 테이블 위의 결과(L1)와 동일한 LOCATION_ID를 가지고 있는 DEPT_ID 조회
SELECT DEPT_ID
FROM DEPARTMENT
...
-- 3) 최종적으로 EMPLOYEE 테이블에서 위의 결과들과 동일한 DEPT_CODE 가진 사원을 조회
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
-- 3. (단일행) 다중열 서브쿼리
-- 		서브쿼리 SELECT 절에 나열된 컬럼 수가 여러개 일 
...
SELECT JOB_CODE = (SELECT JOB_CODE
					FROM EMPLOYEE
					WHERE ENT_YN = 'Y'
					AND SUBSTR(EMP_NO, 8, 1) = '2')
SELECT JOB_CODE = (SELECT JOB_CODE
...
-- 서브커리의 조회된 컬럼과 비교하여 일치하는 행만 조회
-- 컬럼 순서가 중요!!
SELECT EMP_NAME, JOB_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
...
--> 상관 서브쿼리를 사용하여 각 직원의 MANAGER_ID가 실제로 직원 테이블의 EMP_ID가
-- 실제로 직원 테이블의 EMP_ID와 일치하는지 ㄹ학인
			
			
-- 미인쿼리(직원의 사번, 이름, 부서면, 사수사번 조회)
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIB DEPARTMENT ON(DEPT_ID = DEPT_CODE);
WHERE ENT_YN = 'N'; 
...
LEFT JOIB DEPARTMENT ON(DEPT_ID = DEPT_CODE)
--WHERE ENT_YN = 'N'
WHERE HIRE_DATE = (SELECT MIN(HIR_DATE)
				FROM EMPLOYEE SUB
...
-- 메인쿼리
SELECT EMP_NAME, HOB_CODE, SALARY
FROM EMPLOYEE;
...
-- 	서브쿼리가 만든 결과의집함(RESULT SET)을 테이블 대신 사용.
...
SELECT ROWNUM, DEPT CODE, DEPT_TITLE, 평균급여
/* 1*/FROM ( SELECT DEPT_CODE, DEPT_TITLE, CEIL(AVG(SALARY)) 평균급여
		FROM EMPLOYEE
		LEFT JOIN DEPARTMENT ON (DEPT ID = DEPT_CODE)
...
SELECT DENSE RANK() OVER(ORDER BY SALARY DESC) 순위, EMP_NAME, SALARY
FROM EMPLOYEE;
-- 19 	전형돈 2000000
-- 19 	윤은해 2000000
-- 21 	박나라 1800000
...
FROM EMPLOYEE E
JOIN DEPARTMENT D ON E.DEPT_CODE = DEPT_CODE
JOIN JOB J ON E.JOB_CODE = J.JOB_CODE
...
--		사번, 이름, 부서코드, 직급코드, 고용일
SELECT EMP.EMP_ID, EMP.EMP_NAME, EMP.DEPT_CODE, EMP.JOB_CODE, EMP.HIRE_DATE
FROM EMPLOYEE EMP
JOIN DEPARTMENT DEPT ON EMP.DEPT_CODE = DEPT_CODE
JOIN JOB JOB ON EMP.JOB_CODE = JOB.JOB_CODE
WHERE TO_CHAR(EMP.HIRE_DATE, 'YYYY') = '2000';
...
FROM EMPLOYEE E
JOIN EMPLOYEE E2 
  ON E.DEPT_CODE = E2.DEPT_CODE -- 같은 부서
  AND E.MANAGER_ID = E2.MANAGER_ID -- 같은 사수
WHERE E2.EMP_NO LIKE '770%'  -- 1977년생 (주민번호 앞자리)
AND SUBSTR(E2.EMP_NO, 8, 1) = '2';  -- 여성	
...
-- 합침
SELECT DEPT_CODE, EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '소속없음'), JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
SELECT FLOOR (MONTHS_BETWEEN(SYSDATE, (TO DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) /12 ) "나이",
...
-- 서브쿼리
SELECT MIN(FLOOR (MONTHS_BETWEEN(SYSDATE, (TO DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) /12 ))
FROM EMPLOYEE
WHERE JOB_CODE = 'J2';
...
WHERE FLOOR (MONTHS_BETWEEN(SYSDATE, (TO DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) /12 ))
=	SELECT MIN(FLOOR (MONTHS_BETWEEN(SYSDATE, (TO DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) /12 ))
	FROM EMPLOYEE SUB
	WHERE MAIN.JOB_CODE = SUB.JOB_CODE)
ORDER BY "나이" DESC;
