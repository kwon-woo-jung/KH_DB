...
SELECT ROUND(123.456) FROM DUAL; -- 123, 소수점 첫 번쨰 자리에서 반올림

SELECT ROUND(123.456, 1) FROM DUAL;
-- 123.5, 소수점 두번째 자리에서 반올림(소수점 첫번째 자리까지 표기)

SELECT ROUND(123.456, 0) FROM DUAL; -- 소수점 첫 번쨰 자리에서 반올림
...
SELECT TRUNK(123.456) FROM DUAL; -- 123, 소수점 아래를 절삭
SELECT TRUNK(123.456, 1) FROM DUAL; -- 123.4, 소수점 첫째자리 아래 절삭
SELECT TRUNK(123.456, -1) FROM DUAL; -- 120, 10의 자리 아래 절삭
...
SELECT ABS(ROUND(MONTH_BETWEEN(SYSDATE, '2025-01-23'), 3) ) "수강 기간(개월)"
FROM DUAL; -- 3.463
...
EXTRACT(DAT FROM HIRE_DATE) || '일' AS 입사일
FROM EMPLOYEE;
...
-- 2024년 10월 08일 (화)
SELECT TO_CHAR(SYSDATE 'YYYY년 MM"월" DD"일" (DY)') FROM DUAL; 
...
SELECT TO_DATE(20230804) FROM DAUL;
...
SELECT TO_DATE('800505', 'RRMMDD') FROM DUAL; -- 2049-05-05 00:00:00.000
