...
-- 그룹 함수 : N개의 값을 읽어서 연산 후 1개의 결과를 반환 (합계, 평균, )
...
SELECT INSTR ('AABAACAABBA', 'B') FROM DUAL; -- 3

-- 문자열 5번째 문자부터 검색하여 첫번째 B 위치 조회
SELECT INSTR (' AABAACAABBA', 'B', 5) FROM DUAL; -- 9
...
-- 123.5, 소수점 두번쨰 자리에서 반올림(소수점 첫 번째 자리까지 표기)
...
SELECT FLOOR(123.1), FLOOR(123.9) FROM DUAL;
...
-- SYSDATE : 시스템에 현재 시간(년, 월, 일, 시, 분, 초)를 반환
