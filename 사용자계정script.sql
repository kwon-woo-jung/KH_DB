...
-- 1. 이메일 조건:
-- EMAIL LIKE '____#_%' ESCAPE '#': 이메일에서 '_' 앞에 정확히 4글자가
-- 있는 사원을 찾습니다. # 은 ESCAPE 문자로, 뒤에 나오는 '_'를 문자 그대로 인식하도록 합니다.

-- 2. 부서 코드 조건:
-- DEPT_CODE = 'D9' OR DEPT_CODE = 'D6' : 부서 코드가 'D9' 또는 'D6'인 사원들을 찾습니다. OR 연산자는 AND보다 우선순위가 낮기 때
-- 문에 괄호를 사용해 OR조건을 우선적으로 적용했습니다.

-- 3.  입사일 조건:
-- HIRE_DATE BETWEEN '1990-01-01' AND '2000-12-31': 입사일이 1990년
-- 1월 1일에서 2000년 12월 31일 사이에 있는 사원을 찾습니다.

-- 4. 급여 조건:
-- SALARY >= 2700000 : 급여가 270만원 이상인 사원을 필터링합니다.

-- 연산자 우선순위:
-- AND 연산자가 OR 연산자보다 우선순위가 높으므로, 괄호 ()를 사용하여
-- 부서 코드 조건을 묶어 OR 조건이 먼저 족용되도록 했습니다.

----------------------------------------------------------------------

/* 
 해당 코드는 IN 연산자와 NOT IN 연산자의 사용법을 보여줍니다. IN과 NOT IN
 연산자는 다수의 값을 비교할 때 유용하게 사용됩니다.
 ...
 이 쿼리는 **DEPT_CODE**가 'D1', 'D6', 'D9' 중 하나인 사원들을 조회합니다.
 IN 연산자는 여러 값을 효율적으로 비교하는 방법으로, 여러 개의 OR 조건을
 한 줄로 간단하게 표현할 수 있습니다.
 ...
 IN 연산자: 여러 값 중 하나와 일치하는 데이터를 조회할 때 사용합니다. 간
 단한 다중 조건 비교를 제공합니다.
